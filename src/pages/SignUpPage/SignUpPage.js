...
function SignUpPage() {
    ...
    const handleForm = async (e) => {
        ...
        if (inputs.current[5].value !== inputs.current[6].value) {
            ...
            alert("Les mots de passe ne correspondent pas"); // Informer l'utilisateur
            return;
        }

        try {
            // Attendre la résolution de signUp et vérifier son succès
            await signUp(
                inputs.current[2].value, // email
                inputs.current[5].value, // password
                informations
            );
            ...
            alert("Erreur lors de l'inscription : " + error.message); // Informer l'utilisateur
